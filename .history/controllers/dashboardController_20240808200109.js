...
const getMonthlySalesDataForProduct = async (req, res) => {
  const productId = req.params.productId;
  try {
    // Query to find orders containing the specified productId
    const query = {
      "products.productId": productId,
    };

    // Find orders with the specified productId
    const orders = await Order.find(query);
    ...
    orders.forEach((order) => {
      ...
      // Ensure createdAtDate is a valid date
      if (!isNaN(createdAtDate.getTime())) {
        const month = createdAtDate.getMonth() + 1; // Months are 0-based in JS

        // Process each product in the order
        order.products.forEach((product) => {
          if (product.productId.toString() === productId) {
            // Aggregate the quantity for the specified product
    ...
    // Return the monthly sales data as JSON
